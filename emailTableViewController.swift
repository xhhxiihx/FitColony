...
class emailTableViewController: UITableViewController {
    ...
    func save(){
        ...
        email = self.inputbox.text
        if email != nil{
            user.email = self.email!
            user.saveInBackground()
            delegate?.refreshEmail(email!)
        }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.inputbox.text = email
        self.inputbox.becomeFirstResponder()
        self.navigationItem.title="邮箱"
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "save", style: .Plain, target: self, action: #selector(emailTableViewController.save))
